...
`internal`按session、bgp、bmp、rib、rpki、refresh、validation、analysis、distribution、tenant和platform领域拆分；核心领域分别包含domain、application、adapter和infrastructure层。refresh链路负责路由源快照拉取、取消和原子提交，validation链路保留可持久化的路由源验证失败分类，协议解析使用固定消息上限，所有后台循环接受context取消，内存RIB用于离线模式，`migrations/001_init.sql`提供PostgreSQL事件和检查点表供生产仓储实现。
