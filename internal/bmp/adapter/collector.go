...
import (
	...
	"encoding/json"
	"fmt"
	bmpapplication "github.com/routeorigin/route-origin-guardian/internal/bmp/application"
...
type Collector struct {
	...
	admission        *bmpapplication.AdmissionController
	mu               sync.Mutex
}

func NewCollector(s *application.Service, bmp, bgp string) *Collector {
	return &Collector{svc: s, bmpAddr: bmp, bgpAddr: bgp, admission: bmpapplication.DefaultAdmissionController()}
}
...
func (c *Collector) handle(ctx context.Context, conn net.Conn) {
	...
	for scan.Scan() {
		...
		if err := c.admission.Check(ctx, len(line)); err != nil {
